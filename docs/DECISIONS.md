...
**D20 · Gmail-view demo mail is simulated, self-contained and kept out of scoring** (2026-09-22)
`POST /gmail/simulate` builds an inbound email plus matching SI/BL attachments (dataset text format, or image-only PDFs for the scan scenario) in `sdoc/simulate.py`, stores them in the same bucket and runs the normal pipeline, so the demo exercises the real cascade. Ids start with `000_sim_` (sorts first in the ascending inbox; ms timestamp + random tail so rapid clicks cannot collide). They are excluded from `export_submission` and `metrics` so the organiser-format export and the score evidence stay dataset-only, capped at 40 rows, and `DELETE /gmail/simulated` removes rows and files. Carriers/domains are fictional (`.example`). Rejected reusing other emails' attachments: the body would not match the documents.

**D21 · PDF upload on the Process page; demo and uploaded mail now count in metrics** (2026-09-22)
`POST /upload` (multipart, needs `python-multipart`) takes up to 4 PDFs (4 MB total: Vercel rejects larger bodies). Each is typed as email / SI / draft BL from its content (an email header block, else the document heading, else the file name; scans need the user to pick a type). They are stored under `uploads/<id>/` with names ending `_SI.pdf` / `_BL.pdf` so the existing pipeline and classifier handle them unchanged, then the email is processed immediately. Supersedes part of D20: simulated and uploaded emails are now included in `/metrics` and the daily counts (the UI must reflect them; clearing simulated mail lowers the numbers), but `/export/submission` still defaults to the 520 dataset emails so the organiser scorer is unaffected; the UI's JSON download passes `include_extra=true`. `/samples/{kind}` serves generated sample PDFs (`sdoc/samples.py`) so the flow can be tried without files.

**D22 · Route map uses the real ports, not an assumed hub** (2026-09-22)
The first map paired every destination with Singapore because the email list has no ports; that was an unsupported assumption. `GET /metrics/routes` now returns each BL comparison's port of loading / discharge from its stored SI/BL values (SI value, else BL); the map draws loading -> discharge. Emails with no document values (draft-BL requests, ~half of the comparisons) fall back to the destination named in the subject and show as a highlighted point; emails with no port at all are grouped as "Ports not stated" in the lane list only. Metrics-page accuracy figures come from `src/data/validation.json` (scorer output logged in PLAN.md); the live counts under them follow the database.
