...
**Live app: https://monash-hackathon-five.vercel.app** · API docs: `/api/py/docs`
...
confirm or correct a value; the status is recomputed and an audit row is written.

Try it (60 seconds): open the inbox, filter **Status → Mismatch**, open a report; open `email_001` for the all-clear case;
open `email_512` (a scanned PDF) to see AI-suggested values and correct them in the review panel; finish on **Metrics**.
...
Scored with the organiser's scorer (aggregate output only; the answer key is never read).
...
- **Robustness tests.** Synthetic SI/BL pairs are generated in txt/docx/xlsx across three label vocabularies: identical
  content is never flagged; changing one field flags exactly that field; blanks, wrong document types and unit or suffix
  differences (`22 MT` = `22,000 KG`, `LIMITED` = `LTD`) behave correctly.
- **Cost and speed.** 88% of emails are decided by rules with no AI call. Every LLM response is cached by content hash
  ...
  vision calls cost ≈ **$0.009**. Cloud batches of 15 emails take 12-50 s (database round-trips); a cold LLM run is paced by
  the free Groq tier (1,000 output tokens/min).
- **126 automated tests** run in CI on every push (Python 3.12) alongside the frontend's lint, type-check and build.
...
pytest -q                                 # 126 tests, no keys needed
...
sdoc/            pipeline: config, llm, parse/, classify, extract, normalize, compare, decide, adjudicate,
                 explain, pipeline, service, db, store, metrics
src/             Next.js 16 UI: inbox, email report, review panel, review queue, process, metrics
supabase/        schema.sql (tables, RLS on, private bucket created by the seed script)
scripts/         run_pipeline, score, evaluate, seed_supabase, cloud_run, eval_extraction
tests/           pytest: parsers, normalisers, compare/decide, AI layer with fakes, stress, API, unseen phrasings
docs/            SPEC, ARCHITECTURE, DECISIONS, DATA_NOTES, RUBRIC, PLAN, DEMO
```
